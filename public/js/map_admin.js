  ...
  map.on("pointermove", function (evt) {
    ...
    map.on("click", function (event) {
      ...
      if (clusterHoverMarker) {
        var mycluster = feature_onHover.get("geometry").getExtent();
        var actualZoom = Math.floor(map.getView().getZoom())
        var newZoom = actualZoom;
        
        if (actualZoom < 14) newZoom = 14;
        else newZoom = 18;

        map.getView().fit(mycluster, {size:map.getSize(), maxZoom:newZoom, duration:800});
