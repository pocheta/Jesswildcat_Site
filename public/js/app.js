;(function (window, document) {
  ...
  var handleFiles = function (files) {
    ...
    if ( count > 0 ) {
      ...
      for ( i = prev_count_files + waiting, j = 0; i < prev_count_files + files.length + waiting; i++, j++ ) {
        document.getElementById('dropzone').innerHTML += '<div class="file" id="file-' + i + '"><div class="name">' + files[j].name + '</div><div class="clear"></div></div>';
      }
  ...
  var handleReaderLoad = function (evt) {
    ...
    var current_file = {
      name: encodeURI(all_files[current_file_id].name),
